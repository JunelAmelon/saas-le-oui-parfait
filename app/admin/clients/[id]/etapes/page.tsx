...
import { ArrowLeft, CheckCircle, Circle, Loader2, Plus, Trash2, Pencil, RefreshCcw } from 'lucide-react';
...
export default function ClientStepsAdminPage() {
  ...
  const hasAutoGenerated = useMemo(() => steps.some((s) => s.auto_generated), [steps]);

  const generateAutoSteps = async () => {
    if (!eventDate || !eventId || !plannerId) {
      toast.error('Date de mariage ou événement manquant');
      return;
    }

    setIsGenerating(true);
    try {
      const payloads = getWeddingTaskPayloads(clientId, plannerId, eventId, eventDate);
      for (const task of payloads) {
        await addDocument('tasks', task);
      }
      await fetchAll();
      toast.success('Étapes automatiques générées');
    } catch (e) {
      console.error('Error generating auto steps:', e);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <PageHeader title="Étapes" description="Créez et validez les étapes clés pour ce client">
          ...
          {!hasAutoGenerated && eventDate && (
            <Button
              className="bg-[#C9A96E] hover:bg-[#B8975E] text-white w-full sm:w-auto gap-2"
              onClick={() => void generateAutoSteps()}
