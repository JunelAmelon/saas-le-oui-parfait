...
export default function SettingsPage() {
  ...
  const [qontoConnecting, setQontoConnecting] = useState(false);
  const [qontoOnboarding, setQontoOnboarding] = useState(false);
  const [qontoConnectionStatus, setQontoConnectionStatus] = useState<string | null>(null);
  const [qontoAuthUrl, setQontoAuthUrl] = useState<string | null>(null);
  const [qontoOrgInfo, setQontoOrgInfo] = useState<{ organization_id: string | null; bank_accounts: Array<{ id: string; iban: string; status: string; main: boolean }> } | null>(null);
  const [qontoSelectedBankAccountId, setQontoSelectedBankAccountId] = useState<string | null>(null);
  const [qontoPartnerCallbackUrl, setQontoPartnerCallbackUrl] = useState<string>('');
  const [qontoWebsiteUrl, setQontoWebsiteUrl] = useState<string>('');
  ...
  const handleConnectQonto = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(String(data?.error || 'qonto_oauth_start_error'));
      }

      const url = String(data?.url || '');
      if (!url) throw new Error('missing_oauth_url');

      window.location.href = url;
    } catch (e: any) {
      sonnerToast.error(`Erreur Qonto: ${String(e?.message || 'error')}`);
    } finally {
      setQontoConnecting(false);
    }
  };

  const handleFetchQontoOrganization = async () => {
    setQontoOnboarding(true);
    try {
      const idToken = await getIdToken();
      const res = await fetch('/api/qonto/organization', {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${idToken}`,
        },
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(String(data?.error || 'qonto_organization_error'));
      }
      ...
      if (msg === 'not_authenticated') {
        ...
        sonnerToast.error(`Erreur Qonto: ${msg}`);
      }
    } finally {
      setQontoOnboarding(false);
    }
  };

  const handleDebugFetchQontoAuthUrl = async (scopes?: string) => {
    setQontoConnecting(true);
    try {
      const firebaseUser = auth.currentUser;
      if (!firebaseUser) {
      ...
      const res = await fetch('/api/qonto/oauth/start', {
        ...
        body: JSON.stringify({ dryRun: true, ...(scopes ? { scopes } : {}) }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(String(data?.error || 'qonto_oauth_start_error'));
      const url = String(data?.url || '');
      if (!url) throw new Error('missing_oauth_url');
      setQontoAuthUrl(url);
    } catch (e: any) {
  ...
  const handleRefreshQontoConnectionStatus = async () => {
    setQontoOnboarding(true);
    try {
      const idToken = await getIdToken();
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(String(data?.error || 'qonto_connection_status_error'));
      }
      setQontoConnectionStatus(data?.status || null);
    } catch (e: any) {
      const msg = String(e?.message || 'error');
      if (msg === 'not_authenticated') {
        sonnerToast.error('Vous devez être connecté');
  ...
  const handleStartQontoPaymentLinksOnboarding = async () => {
    setQontoOnboarding(true);
    try {
      const idToken = await getIdToken();
      const res = await fetch('/api/qonto/payment-links/connect', {
        ...
        body: JSON.stringify({
          ...(qontoSelectedBankAccountId ? { bankAccountId: qontoSelectedBankAccountId } : {}),
          ...(qontoPartnerCallbackUrl ? { partnerCallbackUrl: qontoPartnerCallbackUrl } : {}),
          ...(qontoWebsiteUrl ? { websiteUrl: qontoWebsiteUrl } : {}),
        }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  useEffect(() => {
    try {
      if (typeof window !== 'undefined') {
        if (!qontoPartnerCallbackUrl) setQontoPartnerCallbackUrl(`${window.location.origin}/settings?tab=integrations`);
        if (!qontoWebsiteUrl) setQontoWebsiteUrl(window.location.origin);
      }
    } catch {
      // ignore
    }
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                  <div className="flex items-center gap-4">
                    <div className="h-12 w-12 bg-brand-turquoise rounded-lg flex items-center justify-center text-white font-bold">
                      Q
                    </div>
                    <div>
                      <p className="font-medium">Qonto</p>
                      <p className="text-sm text-brand-gray">Paiements par lien (carte/virement) via Qonto</p>
                      <p className="text-sm text-brand-gray">
                        Statut Payment Links: {qontoConnectionStatus || 'inconnu'}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      className="border-2 border-brand-turquoise"
                      onClick={handleConnectQonto}
                      disabled={authLoading || qontoConnecting || qontoOnboarding}
                    >
                      {qontoConnecting ? 'Connexion...' : 'Connect with Qonto'}
                    </Button>
                    <Button
                      variant="outline"
                      className="border-2 border-brand-turquoise"
                      onClick={() => handleDebugFetchQontoAuthUrl()}
                      disabled={authLoading || qontoConnecting || qontoOnboarding}
                    >
                      Debug URL
                    </Button>
                    <Button
                      variant="outline"
                      className="border-2 border-brand-turquoise"
                      onClick={handleRefreshQontoConnectionStatus}
                      disabled={authLoading || qontoConnecting || qontoOnboarding}
                    >
                      {qontoOnboarding ? '...' : 'Rafraîchir'}
                    </Button>
                    <Button
                      className="bg-brand-turquoise hover:bg-brand-turquoise-hover"
                      onClick={handleStartQontoPaymentLinksOnboarding}
                      disabled={authLoading || qontoConnecting || qontoOnboarding}
                    >
                      {qontoOnboarding ? 'Activation...' : 'Activer Payment Links'}
                    </Button>
                  </div>
                </div>
                ...
                {qontoAuthUrl ? (
                  <div className="p-4 border border-gray-200 rounded-lg">
                    <p className="text-sm text-brand-gray break-all">{qontoAuthUrl}</p>
                    <div className="mt-2 flex items-center gap-2">
                      <Button
                        ...
                        onClick={() => {
                          try {
                            navigator.clipboard.writeText(qontoAuthUrl);
                      ...
                      <Button
                        variant="outline"
                        className="border-2 border-brand-turquoise"
                        onClick={() => handleDebugFetchQontoAuthUrl('offline_access organization.read')}
                      >
                        Test scopes min
                      </Button>
                    </div>
                ...
                <div className="p-4 border border-gray-200 rounded-lg">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="font-medium text-sm">Bank accounts Qonto</p>
                      <p className="text-xs text-brand-gray">Copie l’ID (UUID) dans QONTO_PAYMENT_LINKS_BANK_ACCOUNT_ID</p>
                    </div>
                    <Button
                      variant="outline"
                      className="border-2 border-brand-turquoise"
                      onClick={handleFetchQontoOrganization}
                      disabled={authLoading || qontoConnecting || qontoOnboarding}
                    >
                      {qontoOnboarding ? '...' : 'Récupérer'}
                    </Button>
                  </div>

                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <p className="text-xs text-brand-gray">partner_callback_url</p>
                      <Input value={qontoPartnerCallbackUrl} onChange={(e) => setQontoPartnerCallbackUrl(e.target.value)} />
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-brand-gray">user_website_url</p>
                      <Input value={qontoWebsiteUrl} onChange={(e) => setQontoWebsiteUrl(e.target.value)} />
                    </div>
                  </div>

                  <p className="mt-2 text-xs text-brand-gray break-all">Sélectionné pour activation: {qontoSelectedBankAccountId || '—'}</p>

                  {qontoOrgInfo ? (
                    <div className="mt-3 space-y-2">
                      <p className="text-xs text-brand-gray break-all">organization_id: {qontoOrgInfo.organization_id || '—'}</p>
                      {qontoOrgInfo.bank_accounts.length ? (
                        <div className="space-y-2">
                          {qontoOrgInfo.bank_accounts.map((b) => (
                            <div key={b.id} className="text-xs text-brand-gray break-all">
                              <div>id: {b.id}</div>
                              <div>iban: {b.iban || '—'} {b.main ? '(main)' : ''}</div>
                              <div className="mt-1">
                                <Button
                                  variant="outline"
                                  className="border-2 border-brand-turquoise h-8 px-2 text-xs"
                                  onClick={() => setQontoSelectedBankAccountId(b.id)}
                                >
                                  Utiliser pour activation
                                </Button>
                              </div>
                            </div>
                          ))}
                        ...
                        <p className="text-xs text-brand-gray">Aucun compte renvoyé</p>
                      )}
                    </div>
                  ) : null}
                </div>
